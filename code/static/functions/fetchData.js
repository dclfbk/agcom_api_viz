...
async function fetchDataSelection(url) {
  if (controller_selection.signal.aborted) return null;                   //if controller is aborted block fetch
  try {
    const response = await fetch(url, {
      signal: controller_selection.signal                                 //while fetching, check if controller gets aborted
    });
    if (!response.ok) {
      throw new Error("Network response was not ok");
    }
